# -*- coding: utf-8 -*-
"""Фактовая часть стартера сессии — генерируется, не пишется руками.
...
templates/СТАРТЕР-шаблон.md). Рассказ пишется поверх фактов; «✅» прошлых
...
def main() -> int:
    ...
    lines.append("> пока не подтверждены прогоном в текущей. Рассказ пишется ПОВЕРХ этого блока.")
    lines.append("> Блок содержит пути машины и сообщения коммитов — при публикации наружу обезличь.")
