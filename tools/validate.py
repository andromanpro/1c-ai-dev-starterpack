...
SECRETS = [
    ...
    # Только строковый ЛИТЕРАЛ: `password = "12345"`. Аннотация типа (`password: str`),
    # передача переменной (`self.password = password`) и чтение из окружения
    # (`os.environ.get("APP_PASSWORD", "")`) — не находки, а нормальный код, и раньше
    # они давали шум, из-за которого проверку начинают игнорировать целиком.
    (re.compile(r"""(пароль|password)\s*[=:]\s*["'][^"']+["']""", re.I), "пароль в открытом виде", True),
]

# Каждый диапазон описан целиком: у 192.168 и 172.16-31 первые два октета уже
# заданы, и добирать нужно ровно два. Прежний общий хвост из трёх октетов давал
# пятичастный адрес, поэтому 192.168.77.55 проверку молча проходил.
PRIVATE_IP = re.compile(
    r"\b(?:192\.168\.(\d{1,3})\.(\d{1,3})"
    r"|172\.(?:1[6-9]|2\d|3[01])\.(\d{1,3})\.(\d{1,3})"
    r"|10\.(\d{1,3})\.(\d{1,3})\.(\d{1,3}))\b"
)
...
PERSONAL = [
    ...
    # Домен верхнего уровня — буквы: иначе под «почту» попадают версии пакетов
    # (three@0.140, 3d-force-graph@1.74) и любая строка вида имя@число.число.
    (re.compile(r"[\w.+-]+@[\w-]+\.[A-Za-z]{2,}"), "адрес почты"),
]
...
def ip_is_real(m: re.Match) -> bool:
    """Отсекает совпадения вроде версий: октет больше 255 — не адрес.

    Проверяются все сработавшие группы, потому что диапазоны описаны отдельными
    ветками и номера групп у них разные.
    """
    parts = [g for g in m.groups() if g is not None]
    try:
        return bool(parts) and all(int(p) <= 255 for p in parts)
    except ValueError:
        return False
...
def _dirty_sample() -> str:
    ...
    return "\n".join([
        ...
        # Литерал в кавычках — именно то, что правило обязано ловить после уточнения
        # (голое `Пароль = переменная` находкой больше не считается: это обычный код).
        'Пароль = "' + "СуперСекрет123" + '"',
        'connect(user=admin, password="' + "hunter2" + '")',
...
def selftest() -> int:
    ...
    # Именованные категории, а не их количество: при проверке «не меньше пяти»
    # выпадение одной (например, внутреннего IP из-за ошибки в шаблоне)
    # маскировалось остальными, и самотест оставался зелёным.
    expected = {"токен GitHub", "личный путь (Users)", "адрес почты",
                "пароль в открытом виде", "внутренний IP-адрес"}
    missing = expected - kinds
    if missing:
        print("САМОТЕСТ ПРОВАЛЕН: грязный образец не дал категорий: " + ", ".join(sorted(missing)))
        ok = False
    if len(dirty_f) < len(expected):
        print(f"САМОТЕСТ ПРОВАЛЕН: находок {len(dirty_f)}, ожидалось не меньше {len(expected)}")
        ok = False
    if clean_f:
