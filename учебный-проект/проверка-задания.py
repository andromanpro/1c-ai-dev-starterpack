# -*- coding: utf-8 -*-
"""Контрольная точка задания 2 учебного проекта.
...
На ИСХОДНОМ модуле проверка обязана быть красной — это её самотест:
  py -3.14 проверка-задания.py --selftest
Обычный запуск:
  py -3.14 проверка-задания.py src/ОбменЗаказами.bsl
"""
import re
import sys
...
if __name__ == "__main__":
    if "--selftest" in sys.argv:
        sys.exit(selftest())
    if len(sys.argv) < 2:
        print("укажите путь к модулю: py -3.14 проверка-задания.py src/ОбменЗаказами.bsl")
        sys.exit(2)
